...
YearlyInflowOutflow <- ggplot() +
  geom_line( aes(x = mean_yearly_data$Year, y= (mean_yearly_data$Yearly_Inflow)/100000000, color= 'Yearly Inflow'))+
   geom_line(aes(x = mean_yearly_data$Year, y= (mean_yearly_data$Yearly_Outflow)/100000000, color= 'Yearly Outflow')) +
  labs(
    title = "Total Inflow and Outflow Over Time",
    x = "Year",
    y = "Billions of gallons",
    color= "Legend", 
  ) +
  scale_x_continuous(breaks = seq(min(mean_yearly_data$Year), max(mean_yearly_data$Year), by = 5)) +  
  scale_y_continuous(breaks = seq(0, max(mean_yearly_data$Yearly_Inflow / 100000000, mean_yearly_data$Yearly_Outflow / 100000000), by = 5)) +  # Customize y-axis
  theme_minimal() +
