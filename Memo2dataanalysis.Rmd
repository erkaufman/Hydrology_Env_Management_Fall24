...
# read in raw data
Greensboro_raw <- read_excel("Home/Hydrology/Data/Hydrology_memo_2.xlsx", 
...
# Function to create separate tables for each timeframe
create_precipitation_table <- function(data_24, data_72, city_name) {
  # Get unique timeframes
  timeframes <- unique(data_24$Timeframe)
  
  # Loop through each timeframe and create a separate table
  for (timeframe in timeframes) {
    # Filter data for the current timeframe
    data_24_tf <- data_24 %>% filter(Timeframe == timeframe)
    data_72_tf <- data_72 %>% filter(Timeframe == timeframe)

    # Combine the 24-hr and 72-hr data
    combined_data <- bind_rows(
      ...
      select(Duration, `10 Year Flood`, `25 Year Flood`, `50 Year Flood`, `100 Year Flood`)

    # Create the table for the current timeframe
    cat(paste("\nPrecipitation frequency estimates (in) for", city_name, "-", timeframe, "\n"))
    
    # Use kable to format the table
    print(kable(combined_data, 
                caption = paste("Precipitation frequency estimates (in) for", city_name, "during", timeframe)) %>%
            kable_styling(full_width = FALSE, position = "center") %>%
            row_spec(0, bold = TRUE) %>%
            column_spec(1, bold = TRUE) %>%
            add_header_above(c(" " = 1, "Recurrence Interval (Years)" = 4)))
  }
}

# Example usage:
# For Boone, NC
create_precipitation_table(final_results_boone_24, final_results_boone_72, "Boone, NC")

# For Greensboro, NC
create_precipitation_table(final_results_greensboro_24, final_results_greensboro_72, "Greensboro, NC")

# For Greenville, NC
create_precipitation_table(final_results_greenville_24, final_results_greenville_72, "Greenville, NC")
...
create_table_inches <- function(data_24, data_72, city_name) {
  ...
  for (timeframe in unique(data_24$Timeframe)) {
    ...
    combined_data <- bind_rows(
      ...
      select(Duration, Probability, Predicted_Precipitation_in) %>%  # Fixed the select statement
      pivot_wider(names_from = Probability, values_from = Predicted_Precipitation_in) %>%
      rename(
        `10 Year Flood` = `10`,  
        `25 Year Flood` = `4`,   
        `50 Year Flood` = `2`,   
...
create_table_inches(final_results_boone_24, final_results_boone_72, "Boone, NC")

# Create tables for Greensboro
create_table_inches(final_results_greensboro_24, final_results_greensboro_72, "Greensboro, NC")

# Create tables for Greenville
create_table_inches(final_results_greenville_24, final_results_greenville_72, "Greenville, NC")


```
[add explanation if anything is changing?]
After getting the precipitation frequency estimates for 24-hr and 72-hr events for 10 year, 25 year, 50 year, and 100 year return intervals, we examined what the maximum monthly 24-hr and 72-hr events were from 1980-1999 and from 2000-2016 in each city. The purpose of this was to see if 25 year flood events (what we are assuming the infrastructure was designed for at each campus in 1999) are becoming more frequent or changing seasons. 
...
greensboro_explore_frontal_post <-Greensboro_24hr %>% 
  filter(year>= 2000,
         `Area Weighted Mean Precipitation (mm per day)`> 24.4,
         month== c(1,2,3,4,5,6))
  mean(greensboro_explore_hurricane_pre$`Area Weighted Mean Precipitation (mm per day)`)
mean(greensboro_explore_hurricane_post$`Area Weighted Mean Precipitation (mm per day)`)
mean(greensboro_explore_frontal_pre$`Area Weighted Mean Precipitation (mm per day)`)
mean(greensboro_explore_frontal_post$`Area Weighted Mean Precipitation (mm per day)`)
